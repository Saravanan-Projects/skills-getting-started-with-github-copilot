    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length > 0) {
          participantsList = `<div class="participants-list">${details.participants.map(p => `
            <span class="participant-item">${p} <span class="delete-icon" title="Remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}">&#128465;</span></span>
          `).join('')}</div>`;
        } else {
